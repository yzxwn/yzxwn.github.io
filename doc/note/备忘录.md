...
性别 工作经历合并 描述20不好 xx平台里的聊天室 不知道换成自己的理解
...
* **JavaScript**：
    ...
    * **执行机制 Event Loop**：
    ...
    JavaScript如何实现异步编程，可以详细描述EventLoop机制：当一个任务执行结束后，当当前的微任务队列为空时，再去执行宏任务队列中的任务
    宏任务和微任务分别有哪些：
        宏任务：setTimeout、setInterval、click、渲染html文档
...
* **Vue**：
    ...
    * **Vue3 proxy 解决了哪些问题，优劣**：
    ...
    3）添加删除属性的问题
    4）新版本将这个模块被耦合到了vue里面，可以单独作为一个库来使用
    ```
    * **发布订阅模式和观察者模式的异同**：
    ```
    ```
